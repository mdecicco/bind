...
namespace bind {
    DataType::Property::Property(
        i32 _offset, DataType::Property::Flags _flags, DataType* _type, const String& _name, AccessFlags _accessFlags
    ) {
        offset      = _offset;
        thisOffset  = 0;
        flags       = _flags;
        ...
        if (flags.is_ctor || flags.is_dtor || flags.is_method || flags.is_pseudo_method || flags.is_static) {
            if (offset != -1) {
                throw InputException("Only non-static object members can have offset >= 0");
            }
        } else {
            if (offset == -1) {
                throw InputException("Non-static object members must have offset >= 0");
            }
        }
    }
    ...
    DataType::Property::Property(const Property& prop)
        : offset(prop.offset),
          ...
          thisOffset(prop.thisOffset),
          flags(prop.flags),
          type(prop.type),
          name(prop.name),
          accessFlags(prop.accessFlags) {}

    DataType::DataType(const String& name, const type_meta& meta, Namespace* ns)
        : ISymbol(
              name, ISymbol::genFullSymbolName(ns, name), ISymbol::genTypeSymbolName(ns, name), SymbolType::DataType
          ),
          m_info(meta),
          m_ownNamespace(new Namespace(ns, this)),
          m_pointerToSelf(nullptr) {
        Registry::Add(m_ownNamespace);
        if (ns) {
            ns->add(this);
        } else {
            Registry::GlobalNamespace()->add(this);
        }
        ...
        if (meta.is_primitive) {
            if (meta.is_integral) {
                if (meta.is_unsigned) {
                    switch (meta.size) {
                        case 1: {
                            m_ffi = ffi_type_uint8;
                            break;
                        }
                        case 2: {
                            m_ffi = ffi_type_uint16;
                            break;
                        }
                        case 4: {
                            m_ffi = ffi_type_uint32;
                            break;
                        }
                        case 8: {
                            m_ffi = ffi_type_uint64;
                            break;
                        }
                    }
                } else {
                    switch (meta.size) {
                        case 1: {
                            m_ffi = ffi_type_sint8;
                            break;
                        }
                        case 2: {
                            m_ffi = ffi_type_sint16;
                            break;
                        }
                        case 4: {
                            m_ffi = ffi_type_sint32;
                            break;
                        }
                        case 8: {
                            m_ffi = ffi_type_sint64;
                            break;
                        }
                    }
                }
            } else if (meta.is_floating_point) {
                if (meta.size == sizeof(f32)) {
                    m_ffi = ffi_type_float;
                } else {
                    m_ffi = ffi_type_double;
                }
            } else if (meta.size == 0) {
                m_ffi = ffi_type_void;
            }
    ...
    const Array<DataType::BaseType>& DataType::getBases() const {
        return m_bases;
    }

    void* DataType::castPointerAsBaseType(void* ptr, const DataType* baseType) const {
        const BaseType* baseInfo = getBaseInfo(baseType);
        if (!baseInfo) {
            throw InputException("'%s' is not a base type of '%s'", baseType->getName().c_str(), getName().c_str());
        }

        return (void*)((u8*)ptr + baseInfo->offset);
    }

    const DataType::BaseType* DataType::getBaseInfo(const DataType* baseType) const {
        for (u32 i = 0; i < m_bases.size(); i++) {
            if (m_bases[i].type == baseType) {
                return &m_bases[i];
            }
        }
    ...
    Array<const DataType::Property*> DataType::findProperties(const String& name, AccessFlags accessMask) const {
        ...
        for (u32 i = 0; i < m_props.size(); i++) {
            ...
            if (p.accessFlags != PublicAccess && (p.accessFlags & accessMask) != p.accessFlags) {
                continue;
            }
            if (p.name == name) {
                ret.push(&p);
    ...
    Array<Function*> DataType::findMethods(const FuncMatch& search, Function** singleStrictMatch) const {
        ...
        for (u32 i = 0; i < m_props.size(); i++) {
            const Property& p = m_props[i];
            if (p.flags.is_method == 0 && p.flags.is_pseudo_method == 0) {
                continue;
            }
            if (p.accessFlags != PublicAccess && (p.accessFlags & search.accessMask) != p.accessFlags) {
                continue;
            }

            FunctionType* sig = (FunctionType*)p.type;

            if (p.name.size() != search.name.size()) {
                continue;
            }
            ...
            if (search.returnType) {
                if (search.returnTypeStrict) {
                    if (!sig->getReturnType()->isEqualTo(search.returnType)) {
                        continue;
                    }
                } else {
                    if (!sig->getReturnType()->isConvertibleTo(search.returnType)) {
                        continue;
                    }

                    if (strictMatchCount <= 1 && singleStrictMatch &&
                        !sig->getReturnType()->isEqualTo(search.returnType)) {
                        isStrictMatch = false;
                    }
                }
            }
            ...
            if (search.doCheckArgs) {
                ...
                if (search.argTypes.size() != args.size()) {
                    continue;
                }
                ...
                for (u32 a = 0; a < args.size(); a++) {
                    ...
                    if (search.argTypesStrict) {
                        ...
                        if (strictMatchCount <= 1 && isStrictMatch && singleStrictMatch &&
                            !args[a].type->isEqualTo(providedType)) {
                            isStrictMatch = false;
                        }
                    }
                }
            ...
            if (isStrictMatch && singleStrictMatch) {
                if (strictMatchCount == 0) {
                    *singleStrictMatch = (Function*)p.address.get();
                } else {
                    *singleStrictMatch = nullptr;
                }
    ...
    Array<Function*> DataType::findConstructors(
        const Array<DataType*>& argTypes, bool strict, AccessFlags accessMask, Function** singleStrictMatch
    ) const {
        if (singleStrictMatch) {
            *singleStrictMatch = nullptr;
        }
        ...
        for (u32 i = 0; i < m_props.size(); i++) {
            ...
            if (p.name.size() != ConstructorNameLen) {
                continue;
            }
            if (p.name != ConstructorName) {
                continue;
            }
            ...
            for (u32 a = 0; a < args.size(); a++) {
                if (strict) {
                    ...
                    if (strictMatchCount <= 1 && isStrictMatch && singleStrictMatch &&
                        !args[a].type->isEqualTo(argTypes[a])) {
                        isStrictMatch = false;
                    }
                }
            }
    ...
    Function* DataType::findConversionOperator(DataType* resultType, AccessFlags accessMask) const {
        for (u32 i = 0; i < m_props.size(); i++) {
            ...
            if (((Function*)p.address.get())->getExplicitArgs().size() != 0) {
                continue;
            }
            if (p.name.size() != CastOperatorNameLen) {
                continue;
            }
            if (p.name != CastOperatorName) {
                continue;
            }

            if (!sig->getReturnType()->isEqualTo(resultType)) {
                continue;
            }
    ...
    bool DataType::isConvertibleTo(DataType* _to, AccessFlags accessMask) const {
        ...
        if (m_info.is_primitive && to->m_info.is_primitive) {
            return true;
        }
        if (m_info.is_pointer && to->m_info.is_pointer) {
            return true;
        }
        ...
        auto castMethods = findMethods(FuncMatch(CastOperatorName).retTp(to, true).noArgs().access(accessMask));

        // conversion via cast operator
        if (castMethods.size() == 1) {
            return true;
        }
    ...
    DataType* DataType::getEffectiveType() const {
        if (m_info.is_alias == 0) {
            return const_cast<DataType*>(this);
        }
