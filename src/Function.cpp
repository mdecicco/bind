...
namespace bind {
    Function::Function(const String& name, FunctionType* sig, Namespace* ns)
        : ISymbol(
              name,
              ISymbol::genFullSymbolName(ns, name),
              ISymbol::genFuncSymbolName(ns, name, sig),
              SymbolType::Function
          ),
          m_signature(sig),
    ...
    void Function::call(void* retDest, void** args) {
        if (!m_callHandler) {
            throw InvalidActionException("Function::call - function '%s' has no call handler", getFullName().c_str());
        }
